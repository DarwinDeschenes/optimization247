[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/4R1Rx8jS)
...
This assignment has 5 parts, and each of the 5 parts is focused on optimizing a program that is given to you. You'll apply the optimization techniques you have learned in order to speed up a program anywhere from 1.5X to 100X. While it's not very often that you have to hand code assembler in order to speed up a program, it is still very common to **optimize your C code** to make it run faster.
...
#### MacOS
Unfortunately gprof is not available on MacOS, but there are alternatives. The easiest to use is Google's perf tools and jeprof. Install these with
```
...
You initialize a matrix with `void transpose_fill(int* matrix, int rows, int cols)` which fills the matrix it with random values.
...
#### Hint
Note that **elements which are close together in the `src` matrix are also close together in the `dst` matrix**, except of course one is close row-wise
and the other is close column-wise. However, you can still use this information to improve speed.
